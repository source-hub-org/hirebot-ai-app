...
type ValidatorFn = (value: string | number, ruleValue?: string | number, ctx?: ValidationContext) => string;
...
const validators: Record<string, ValidatorFn> = {
  ...
  phone: (value, _, ctx) => {
    if (!/^(0|\+84)[1-9][0-9]{8}$/.test(value.toString())) {
      return ctx?.title ? `${ctx.title} không hợp lệ` : 'Số điện thoại không hợp lệ';
    }
    return '';
  },
  email: (value, _, ctx) => {
