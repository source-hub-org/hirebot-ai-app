...
} from "@/stores/candidateDetailSlice";
    ...
    try {
      ...
      if (formData.type === TYPES[0].value) {
        existingQuestionIds =
          store
            .getState()
            .candidateDetail.candidate?.answers?.filter(
              (answer) =>
                answer.language === formData.language &&
                answer.position?.toLowerCase() ===
                  formData.position.toLowerCase() &&
      ...
      if (formData.type === TYPES[2].value) {
        existingQuestionIds =
          store
            .getState()
            .candidateDetail.candidate?.answers?.filter((answer) => answer._id)
            .map((answer) => answer._id)
            .filter(Boolean)
            .join(",") ?? "";
